...
public class NookureStaff {
    ...
    private void loadListeners() {
        ...
        Bukkit.getMessenger()
                .registerIncomingPluginChannel(
                        plugin, Channels.EVENTS, injector.getInstance(BackendMessageMessenger.class));
    ...
    private void loadBukkitListeners() {
        Stream.of(OnPlayerJoin.class, OnPlayerLeave.class, OnPlayerInventoryClick.class)
        ...
        if (config.get().modules.isVanish() && config.get().staffMode.vanishType() == VanishType.INTERNAL_VANISH) {
            Stream.of(PlayerVanishListener.class, StaffVanishListener.class).forEach(this::registerListener);
        }

        if (config.get().modules.isFreeze()) {
            Stream.of(OnFreezePlayerInteract.class, OnFreezePlayerMove.class, OnFreezePlayerQuit.class)
                    .forEach(this::registerListener);
        ...
        if (config.get().playerActions.shiftAndRightClickToInspect()) {
            if (config.get().modules.isPlayerList()
                    && config.get().modules.isPlayerData()
                    && config.get().modules.isUserNotes()) {
                registerListener(OnShiftAndRightClick.class);
            } else {
                logger.warning(
                        "PlayerActions module requires PlayerList, PlayerData and UserNotes modules to be enabled, disabling PlayerActions module");
            }
        }
    ...
    public void unregisterListeners() {
        ...
        closeableListeners.forEach(closeable -> {
            try {
                ...
                logger.severe(
                        "An error occurred while closing listener %s, %s",
    ...
    private void loadTasks() {
        if (config.get().modules.isFreeze() && config.get().freeze.freezeTimer() != -1) {
            Bukkit.getScheduler()
                    .runTaskTimerAsynchronously(plugin, injector.getInstance(FreezeTimerTask.class), 0, 20);
        }

        if (config.get().modules.isFreeze()) {
            Bukkit.getScheduler()
                    .runTaskTimerAsynchronously(plugin, injector.getInstance(FreezeSpamMessage.class), 0, 20 * 5);
        }

        if (config.get().staffMode.actionBar()) {
            Bukkit.getScheduler()
                    .runTaskTimerAsynchronously(plugin, injector.getInstance(StaffModeActionbar.class), 0, 20);
        }
    ...
    private void loadCommands() {
        ...
        if (config.get().modules.isPlayerList()) {
            if (!config.get().modules.isPlayerData()) {
                logger.severe(
                        "PlayerList module requires PlayerData module to be enabled, disabling PlayerList module");
            } else {
                if (!config.get().modules.isUserNotes()) {
                    logger.warning(
                            "PlayerList module requires UserNotes module to be enabled, some features may not work");
                }
    ...
    private void loadExtensions() {
        if (config.get().modules.isFreeze()) extensionManager.registerExtension(FreezePlayerExtension.class);
