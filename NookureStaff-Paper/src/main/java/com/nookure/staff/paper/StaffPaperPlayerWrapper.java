...
public class StaffPaperPlayerWrapper extends PaperPlayerWrapper implements StaffPlayerWrapper {
    ...
    public StaffPaperPlayerWrapper(
        ...
        this.injector = nookPlugin
                .getInjector()
                .createChildInjector(new StaffPaperPlayerWrapperModule(
                        this, serverStaffModeData, staffDataModel, staffMode, staffChatAsDefault));
    ...
    public void loadDBState() {
        if (staffDataModel.get() == null) {
            ...
            if (staffDataModel.get() == null) {
                staffDataModel.set(StaffStateModel.builder()
    ...
    public void writeVanishState(boolean state) {
        staffDataModel.set(
                StaffStateModel.builder(staffDataModel.get()).vanished(state).build());

        staffStateRepository
                .saveOrUpdateModelAsync(staffDataModel.get())
                .thenRun(() -> logger.debug(
                        "Vanish state for %s has been set to %s on the database", player.getName(), state));
    ...
    public void setStaffChatAsDefault(boolean staffChatAsDefault, boolean saveInDB) {
        ...
        staffStateRepository
                .saveOrUpdateModelAsync(staffDataModel.get())
                .thenRun(() -> logger.debug(
                        "Staff chat state for %s has been set to %s on the database",
                        player.getName(), staffChatAsDefault));
    }
    // </editor-fold>
    ...
    public void toggleNightVision() {
        ...
        scheduler.sync(() -> player.addPotionEffect(
    ...
    @Nullable private PotionEffectType getEffect(@NotNull final String effect) {
        ...
        PotionEffectType potionEffectType =
                Registry.EFFECT.get(Objects.requireNonNull(NamespacedKey.fromString(split[0].toLowerCase())));
    ...
    public void addActionBar() {
        ...
        sendActionbar(MiniMessage.miniMessage()
                .deserialize(TextUtils.parsePlaceholdersWithPAPI(
                        player,
                        messages.get()
                                .staffMode
                                .actionBar()
    ...
    public void addExtensions() {
        extensionManager.getExtensionsStream().forEach(extension -> {
            try {
                ...
                logger.severe(
                        "An error occurred while adding extension %s for %s: %s",
                        extension.base().getName(), player.getName(), e.getMessage());
            }
        });
